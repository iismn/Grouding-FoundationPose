...
class TriggerManager(Node):
    def __init__(self):
        ...
        self.declare_parameter('segmentA_launch_args', [
            'text_prompt:=white plastic connector consist with grey part.',
        ])
        self.declare_parameter('segmentB_launch_args', [
            'text_prompt:=white plastic connector consist with grey part.',
        ])
        ...
        self.get_logger().info("[POSE ESTIMATOR] 커넥터 Segment 센서 트리거 대기")

    def trigger_callback(self, msg: String):
        self.get_logger().info(f"[POSE ESTIMATOR] 커넥터 Segment 옵션: {msg.data}")
        if msg.data in ['Alice', 'Bob']:
            self.desired_camera = msg.data
            self.trigger_end = False
        else:
            self.get_logger().warn(f"[POSE ESTIMATOR] 커넥터 Segment 트리거 오류: {msg.data}")
    ...
    def timer_callback(self):
        ...
        if self.active_process is None and self.desired_camera is not None:
            if self.desired_camera == 'Alice':
                time.sleep(3)
                self.get_logger().info("[POSE ESTIMATOR] A_Side 커넥터 Segment 시작")
                launch_args = self.get_parameter('segmentA_launch_args').get_parameter_value().string_array_value
                self.active_process = subprocess.Popen(
                    ['ros2', 'launch', 'connector_semantic_segment', 'semantic_segmentor_launch.py'] + launch_args,
                    preexec_fn=os.setsid
                )
            elif self.desired_camera == 'Bob':
                time.sleep(3)
                self.get_logger().info("[POSE ESTIMATOR] L515 DPT Sensor B (Bob) 시작")
                launch_args = self.get_parameter('segmentB_launch_args').get_parameter_value().string_array_value
                self.active_process = subprocess.Popen(
                    ['ros2', 'launch', 'connector_semantic_segment', 'semantic_segmentor_launch.py'] + launch_args,
                    preexec_fn=os.setsid
                )
        ...
        if self.active_process is not None and self.trigger_end:
            self.get_logger().warn("[POSE ESTIMATOR] 커넥터 Segment 종료")
            try:
