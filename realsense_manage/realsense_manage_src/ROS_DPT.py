...
class TriggerManager(Node):
    def __init__(self):
        super().__init__('trigger_manager')
        self.declare_parameter('camera1_launch_args', ['camera_name:=L515_A', 'serial_no:=f1422241', 'output:=log'])
        self.declare_parameter('camera2_launch_args', ['camera_name:=L515_B', 'serial_no:=f1422940', 'output:=log'])
        ...
        self.get_logger().info("[POSE ESTIMATOR] L515 DPT 센서 트리거 대기")

    def trigger_callback(self, msg: String):
        self.get_logger().info(f"[POSE ESTIMATOR] L515 DPT 옵션: {msg.data}")
        if msg.data in ['Alice', 'Bob']:
            self.desired_camera = msg.data
            self.trigger_end = False
        else:
            self.get_logger().warn(f"[POSE ESTIMATOR] L515 DPT 트리거 오류: {msg.data}")
    ...
    def timer_callback(self):
        ...
        if self.active_process is None and self.desired_camera is not None:
            if self.desired_camera == 'Alice':
                self.get_logger().info("[POSE ESTIMATOR] L515 DPT Sensor A (Alice) 시작")
                launch_args = self.get_parameter('camera1_launch_args').get_parameter_value().string_array_value
                ...
                launch_args = self.get_parameter('camera2_launch_args').get_parameter_value().string_array_value
                self.active_process = subprocess.Popen(
                    ['ros2', 'launch', 'realsense2_camera', 'rs_launch.py'] + launch_args,
                    preexec_fn=os.setsid
                )
        ...
        if self.active_process is not None and self.trigger_end:
            self.get_logger().warn("[POSE ESTIMATOR] L515 DPT Sensor 종료")
            try:
